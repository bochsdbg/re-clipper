...
public:
    ...
    bool SaveToFile(const string& filename, double scale = 1.0, int margin = 10) {
        ...
        file.setf(ios::fixed);
        file.precision(0);
        file << svg_xml_start[0] << ((rec.right - rec.left) + margin * 2) << "px" << svg_xml_start[1] << ((rec.bottom - rec.top) + margin * 2) << "px"
             << svg_xml_start[2] << ((rec.right - rec.left) + margin * 2) << " " << ((rec.bottom - rec.top) + margin * 2) << svg_xml_start[3];
        ...
        for (PolyInfoList::size_type i = 0; i < polyInfos.size(); ++i) {
            ...
            for (Paths::size_type j = 0; j < polyInfos[i].paths.size(); ++j) {
                ...
                file << " M " << ((double)polyInfos[i].paths[j][0].X * scale + offsetX) << " " << ((double)polyInfos[i].paths[j][0].Y * scale + offsetY);
            ...
            file << poly_end[0] << ColorToHtml(polyInfos[i].si.brushClr) << poly_end[1] << GetAlphaAsFrac(polyInfos[i].si.brushClr) << poly_end[2]
                 << (polyInfos[i].si.pft == pftEvenOdd ? "evenodd" : "nonzero") << poly_end[3] << ColorToHtml(polyInfos[i].si.penClr) << poly_end[4]
                 << GetAlphaAsFrac(polyInfos[i].si.penClr) << poly_end[5] << polyInfos[i].si.penWidth << poly_end[6];
            ...
            if (polyInfos[i].si.showCoords) {
                ...
                for (Paths::size_type j = 0; j < polyInfos[i].paths.size(); ++j) {
                    ...
                    for (Path::size_type k = 0; k < polyInfos[i].paths[j].size(); ++k) {
                        IntPoint ip = polyInfos[i].paths[j][k];
                        file << "<text x=\"" << (int)(ip.X * scale + offsetX) << "\" y=\"" << (int)(ip.Y * scale + offsetY) << "\">" << ip.X << "," << ip.Y
                             << "</text>\n";
                        file << "\n";
                    }
                }
...
const std::string SVGBuilder::svg_xml_start[] = {
    ...
    "<svg width=\"",
    "\" height=\"", "\" viewBox=\"0 0 ", "\" version=\"1.0\" xmlns=\"http://www.w3.org/2000/svg\">\n\n"};
...
bool SaveToFile(const string& filename, Paths& ppg, double scale = 1.0, unsigned decimal_places = 0) {
    ...
    for (size_t i = 0; i < ppg.size(); ++i) {
        for (size_t j = 0; j < ppg[i].size(); ++j) ofs << ppg[i][j].X / scale << ", " << ppg[i][j].Y / scale << "," << std::endl;
        ofs << std::endl;
    }
    ofs.close();
    return true;
...
int main(int argc, char* argv[]) {
    if (argc > 1 && (strcmp(argv[1], "-b") == 0 || strcmp(argv[1], "--benchmark") == 0)) {
        ...
        for (int i = 0; i < loop_cnt; i++) {
            ...
            if (!clpr.Execute(ctIntersection, solution, pftEvenOdd, pftEvenOdd)) error_cnt++;
        }
    ...
    if (argc < 3) {
        cout << "\nUsage:\n"
             << "  " << argv[0] << " S_FILE C_FILE CT [S_FILL C_FILL] [PRECISION] [SVG_SCALE]\n"
             << "or\n"

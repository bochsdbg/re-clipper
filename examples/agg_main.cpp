...
public:
    spiral(double x, double y, double r1, double r2, double step, double start_angle = 0)
        : m_x(x), m_y(y), m_r1(r1), m_r2(r2), m_step(step), m_start_angle(start_angle), m_angle(start_angle), m_da(agg::deg2rad(4.0)), m_dr(m_step / 90.0) {}
...
public:
    the_application(agg::pix_format_e format, bool flip_y)
        : agg::platform_support(format, flip_y), m_polygons(5.0, 5.0, 5.0 + 205.0, 110.0, !flip_y), m_operation(555.0, 5.0, 555.0 + 80.0, 130.0, !flip_y) {
    ...
    void perform_rendering(Scanline& sl, Ras& ras, Ren& ren, Clp& clp) {
        if (m_operation.cur_item() > 0) {
            ...
            char buf[100];
            snprintf(buf, 99, "Contours: %d      Points: %d", counter.m_contours, counter.m_points);
            agg::gsv_text txt;
            ...
            snprintf(buf, 99, "Clipper=%.3fms Render=%.3fms", t1, t2);
    ...
    unsigned render_clipper(Scanline& sl, Ras& ras) {
        ...
        case 1: {
            ...
            agg::conv_clipper<agg::path_storage, agg::conv_stroke<agg::path_storage> > clp(ps1, stroke, agg::clipper_or, agg::clipper_non_zero,
        ...
        case 2: {
            ...
            agg::conv_clipper<agg::conv_transform<agg::path_storage>, agg::conv_transform<agg::path_storage> > clp(
                trans_gb_poly, trans_arrows, agg::clipper_or, agg::clipper_non_zero, agg::clipper_non_zero);
        ...
        case 3: {
            ...
            agg::conv_clipper<agg::conv_transform<agg::path_storage>, agg::conv_stroke<spiral> > clp(trans_gb_poly, stroke, agg::clipper_or,
        ...
        case 4: {
            ...
            agg::conv_clipper<agg::conv_stroke<spiral>, agg::conv_curve<agg::conv_transform<agg::path_storage> > > clp(
                stroke, curve, agg::clipper_or, agg::clipper_non_zero, agg::clipper_non_zero);
